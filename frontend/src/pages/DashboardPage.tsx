...
import { formatOriginalTime, formatRescheduledAt, formatTimeRange, toDateOnly, todayDateOnly } from './calendar/dateUtils'
...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const requests: Promise<unknown>[] = [
      ...
      // зависимости от видимости — правило "скрытый виджет не рендерится" применяется только
  ...
  return (
    <section>
      ...
      <div className="dashboard-grid">
        ...
        {isAdmin && visibleWidgets.has('monthly-revenue') && (
          <div className="dashboard-card">
      ...
      {visibleWidgets.has('daily-timeline') && (
        <>
                    ...
                    <div className="dashboard-timeline-track">
                      {row.blocks.map(({ booking, leftPercent, widthPercent }) => {
                        ...
                        return (
                          <div
                            key={booking.id}
                            className={`dashboard-timeline-block${originalTimeLabel ? ' dashboard-timeline-block--rescheduled' : ''}`}
                            style={{
                              left: `${leftPercent}%`,
                              width: `${widthPercent}%`,
      ...
      {visibleWidgets.has('weekly-timeline') && (
        <>
      ...
      {visibleWidgets.has('upcoming-bookings') && (
        <>
